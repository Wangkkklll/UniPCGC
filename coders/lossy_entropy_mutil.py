    ...
    def encode(self,x,th,scale,if_slne,if_mutil,postfix=''):
        ...
        if(if_slne):
            if(if_mutil==True):
                ...
                x_enc = self.sopa_slne.slne.encoder(ground_truth_list[0],complexity)
                x_enc = sort_spare_tensor(x_enc)
                x_enc_F = x_enc.F*bm_data
                x_enc = ME.SparseTensor(
            		features=x_enc_F,
            		coordinates=x_enc.C,
            		coordinate_manager=x_enc.coordinate_manager,
            		tensor_stride=x_enc.tensor_stride)
        ...
        if(scale==2):
            if(if_slne):
                ...
                if(if_mutil==True):
                    ...
                    #y_F = y_F.to(device)
                    y_F = y.F/bm_data
                    y = ME.SparseTensor(features=y_F, coordinates=y.C,
                                tensor_stride=y.tensor_stride, device=device)
                x_down_hat = self.sopa_slne.slne.decoder(y,complexity)
